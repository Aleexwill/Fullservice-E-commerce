    ...
    <>
      ...
      <div className="border-b border-steel-900/40">
        <div className="container-main flex flex-wrap items-center gap-2 py-3 font-body text-caption text-steel-500">
          ...
          <ChevronRight className="h-3 w-3" />
          <span className="text-[#0B1120]">{product.name}</span>
        </div>
      </div>
        ...
        <div className="container-main">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
            ...
            <div>
              ...
              <h1 className="mt-1 font-display text-h1 text-[#0B1120]">{product.name}</h1>
              ...
              <div className="mt-5 flex items-center gap-3">
                <span className="font-display text-h1 text-[#0B1120]">{formatPrice(displayPrice)}</span>
                {strikePrice && (
          ...
          {product.description && (
            <div className="mt-16 max-w-3xl">
              <h2 className="font-display text-h3 uppercase text-[#0B1120]">Descripción</h2>
          ...
          {product.specifications && Object.keys(product.specifications).length > 0 && (
            <div className="mt-10 max-w-3xl">
              <h2 className="font-display text-h3 uppercase text-[#0B1120]">Especificaciones</h2>
              <div className="mt-3 divide-y divide-steel-900/40 rounded-lg border border-steel-900/40">
                {Object.entries(product.specifications).map(([key, value]) => (
                  <div key={key} className="flex justify-between gap-4 px-4 py-2.5 font-body text-body-sm">
                    ...
                    <span className="text-[#0B1120]">{value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {related.length > 0 && (
            <div className="mt-16">
              <h2 className="font-display text-h2 uppercase text-[#0B1120]">También te puede interesar</h2>
                ...
                {related.map((p) => {
                  ...
                  return (
                    <Link key={p.id} href={`/tienda/${p.slug}`} className="card-interactive group overflow-hidden">
                      ...
                      <div className="p-3">
                        <h3 className="font-body text-body-sm font-semibold text-[#0B1120] line-clamp-2">{p.name}</h3>
                        <p className="mt-1 font-display text-body font-bold text-[#0B1120]">{formatPrice(rPrice)}</p>
                      </div>
                    </Link>
                  );
                })}
